...
async def do_login(request: Request, name: str = Form(), code: str = Form()):
    ...
    # 슈퍼관리자 비밀번호 체크 (슈퍼비번이면 아이디(mem_MbrId)로 검색)
    is_super = bool(settings.SUPER_ADMIN_PASSWORD and code == settings.SUPER_ADMIN_PASSWORD)

    # ek_Member에서 인증
    md5_code = hashlib.md5(code.encode()).hexdigest()
    try:
        from app.database import DB_MODE
        # execute_query로 통합 (개발/프로덕션 모두 커넥션 풀 사용)
        if is_super:
            row = execute_query(
                "SELECT mem_MbrId, mem_MbrName, mem_nickname FROM ek_Member "
                "WHERE mem_MbrId = ? LIMIT 1",
                (name,),
                fetch="one"
            )
        else:
            if DB_MODE == "development":
                ...
                row = execute_query(
                    "SELECT mem_MbrId, mem_MbrName, mem_nickname FROM ek_Member "
                    "WHERE mem_MbrName = ? AND mem_pwd = ? LIMIT 1",
                    (name, md5_code),
                    fetch="one"
                )
        if not row:
...
async def messages_page_redirect(request: Request):
    """메시지 목록 → /message/ 로 리다이렉트 (기존 URL 호환)"""
    return RedirectResponse("/message/", status_code=301)


@router.get("/chat/{teacher_id}", response_class=HTMLResponse)
async def chat_page_redirect(teacher_id: str, request: Request):
    """채팅 → /message/chat/ 로 리다이렉트 (기존 URL 호환)"""
    return RedirectResponse(f"/message/chat/{teacher_id}", status_code=301)
