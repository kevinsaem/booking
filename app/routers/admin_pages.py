...
async def admin_stats(request: Request):
    ...
    from app.services.pageview_service import LANDING_PAGES
    ...
    rows = execute_query("""
        SELECT pv_path,
               SUM(CASE WHEN created_at >= ? THEN 1 ELSE 0 END) as cnt_today,
    ...
    page_stats = [
        {**r, "label": LANDING_PAGES.get(r["pv_path"], r["pv_path"])}
        for r in rows
    ]
    totals = {
        "today": sum(r["cnt_today"] for r in rows),
        "d7": sum(r["cnt_7d"] for r in rows),
        "d30": sum(r["cnt_30d"] for r in rows),
        "total": sum(r["cnt_total"] for r in rows),
    }

    # 최근 14일 일별 방문 수 (막대 그래프용)
    ...
    daily = [
        {
            ...
            "short": (now - timedelta(days=i)).strftime("%m/%d"),
            "cnt": daily_map.get((now - timedelta(days=i)).strftime("%Y-%m-%d"), 0),
        }
        for i in range(13, -1, -1)
    ]
    daily_max = max([d["cnt"] for d in daily] + [1])

    # 외부 유입 경로 상위 10건 (최근 30일, 자체 사이트 이동 제외)
    referrers = execute_query("""
        SELECT pv_referrer, COUNT(*) as cnt
        FROM ek_PageView
        WHERE created_at >= ? AND pv_referrer <> '' AND pv_referrer NOT LIKE '%kevinsaem%'
    ...
    ctx.update(_get_flash(request))
    return templates.TemplateResponse(request, "admin/stats.html", ctx)
