...
def check_db():
    ...
    if not os.path.exists(DB_PATH):
        print("❌ dev.db 파일이 없습니다. 먼저 리셋하세요.")
        return
...
def show_stats():
    ...
    print("\n👤 수강생 목록:")
    c.execute("SELECT mem_MbrId, mem_MbrName, injeung_code FROM ek_Member WHERE mem_MbrType='2'")
    for r in c.fetchall():
        print(f"  {r['mem_MbrId']} | {r['mem_MbrName']} | 인증: {r['injeung_code']}")

    print("\n👨‍🏫 강사 목록:")
    c.execute("SELECT mem_MbrId, mem_MbrName FROM ek_Member WHERE mem_MbrType='4'")
    for r in c.fetchall():
